...
def helper(dis):
    ...
    dit = diets[diets['Disease'] == dis]['Diet']
    dit = [dit for dit in dit.values]

    wrkt = workout[workout['disease'] == dis]['workout']

    return desc, pre, med, dit, wrkt
...
# ==================== STREAMLIT APP ==================== #
st.set_page_config(page_title="Medical Recommendation System", page_icon="💊", layout="centered")

st.title("💊 Medical Recommendation System")
st.write("Enter your symptoms to get disease predictions and recommendations.")
...
if st.button("Predict Disease"):
    if not selected_symptoms:
        ...
        desc, pre, med, dit, wrkt = helper(predicted_disease)
        ...
        for i, d in enumerate(dit, start=1):
            st.write(f"{i}. {d}")

        st.subheader("🏃 Workouts")
        for i, w in enumerate(wrkt, start=1):
            st.write(f"{i}. {w}")
